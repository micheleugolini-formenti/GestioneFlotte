...
# --- 1. DATABASE STRUTTURA FLOTTA E ASSEGNATARI (Dati reali dalle immagini) ---
fleet_db = {
...
try:
    ...
    st.info("👉 Assicurati di aver inserito la stringa 'url' nei 'Secrets' di Streamlit Cloud come indicato nella guida.")
    st.stop()

# Creazione automatica della tabella sul Cloud se non esiste
with conn.session as session:
    session.execute("CREATE TABLE IF NOT EXISTS prenotazioni (chiave_mese VARCHAR(50), riga_giorno VARCHAR(50), mezzo VARCHAR(100), tecnico VARCHAR(100), PRIMARY KEY (chiave_mese, riga_giorno, mezzo));")
    session.commit()
...
if st.session_state.page == "home":
    st.title(" Vans Formenti Fleet Cloud System v5")
    ...
    mesi_ita = ["Gennaio", "Febbraio", "Marzo", "Aprile", "Maggio", "Giugno", "Luglio", "Agosto", "Settembre", "Octobre", "Novembre", "Dicembre"]
    chiave_mese = f"{mesi_ita[data_sel.month - 1]}_{data_sel.year}"
    
    res_singolo = conn.query("SELECT tecnico FROM prenotazioni WHERE chiave_mese = :mese AND riga_giorno = :riga AND mezzo = :mezzo", params={"mese": chiave_mese, "riga": label_riga, "mezzo": veicolo_sel})
    ...
    if stato_attuale == "" or pd.isna(stato_attuale):
        ...
        if st.button("Registra Prenotazione sul Cloud"):
            if nome_pren:
                with conn.session as session:
                    session.execute("INSERT INTO prenotazioni (chiave_mese, riga_giorno, mezzo, tecnico) VALUES (:mese, :riga, :mezzo, :tecnico) ON CONFLICT (chiave_mese, riga_giorno, mezzo) DO UPDATE SET tecnico = :tecnico", {"mese": chiave_mese, "riga": label_riga, "mezzo": veicolo_sel, "tecnico": nome_pren})
                    session.commit()
                ...
                for mezzo in veicoli_da_mostrare:
                    ...
                    if valore_nuovo != df_filtrato.at[riga, mezzo]:
                        if valore_nuovo == "" or pd.isna(valore_nuovo):
                            session.execute("DELETE FROM prenotazioni WHERE chiave_mese=:mese AND riga_giorno=:riga AND mezzo=:mezzo", {"mese": f"{mese_nome}_{anno}", "riga": riga, "mezzo": mezzo})
                        else:
                            session.execute("INSERT INTO prenotazioni (chiave_mese, riga_giorno, mezzo, tecnico) VALUES (:mese, :riga, :mezzo, :tecnico) ON CONFLICT (chiave_mese, riga_giorno, mezzo) DO UPDATE SET tecnico = :tecnico", {"mese": f"{mese_nome}_{anno}", "riga": riga, "mezzo": mezzo, "tecnico": valore_nuovo})
            session.commit()
